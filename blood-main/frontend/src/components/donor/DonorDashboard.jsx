            ...
            {nearbyRequests.map((req) => (
              <div key={req.request_id} className="card-panel-highlight p-4 space-y-3">
                ...
                <div className="flex items-center justify-between text-xs text-slate-300 pt-2 border-t border-slate-700/60">
                  <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-red-400" /> {req.distance_km ?? '—'} km away</span>
                  <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-amber-400" /> ETA: ~{req.eta_minutes || 14} mins</span>
                </div>

                <button onClick={async () => {
                  if (req.status === 'Donor Accepted' || req.status === 'Donor Arriving') {
                    if (onAcceptRequest) onAcceptRequest(req.request_id); else onNavigate('live-tracking');
                    return;
                  }
                  try {
                    const response = await fetch('/api/donor/request/accept', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ request_id: req.request_id, user_id: user?.user_id || 'usr_donor_1' }) });
                    const data = await response.json();
                    if (!data.success) { fetchDashboardData(); return; }
                    if (onAcceptRequest) onAcceptRequest(req.request_id); else onNavigate('live-tracking');
                  } catch (e) { console.error(e); }
                }} className="w-full btn-primary justify-center text-xs py-2 mt-1">
                  <Zap className="w-4 h-4" /> {req.status === 'Donor Accepted' || req.status === 'Donor Arriving' ? 'Continue Navigation' : 'Accept & Navigate to Hospital'}
                </button>
              </div>
            ))}
          </div>
