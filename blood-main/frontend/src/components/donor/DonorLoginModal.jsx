...
import { X, Lock, Mail, User, Phone, MapPin, Heart, Shield, Building2, Droplet, Navigation } from 'lucide-react';
...
const DonorLoginModal = ({ isOpen, onClose, targetRole = 'donor', onLoginSuccess }) => {
  ...
  const captureLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords }) => {
        ...
        setLocationStatus(`Current location captured (accuracy about ${Math.round(coords.accuracy)} m).`);
      },
      () => setLocationStatus('Location was not captured. Enable location permission and try again.'),
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === 'register' && role !== 'admin' && !coordinates) {
      setLoading(false);
      setMessage('Please capture your current location before creating the account.');
      return;
    }
            ...
            <>
              ...
              {role !== 'admin' && (
                <div className="form-group">
                  ...
                  <button type="button" onClick={captureLocation} className="form-control text-left flex items-center gap-2 hover:border-sky-500/60">
                    <Navigation className="w-4 h-4 text-sky-400" />
                    {coordinates ? 'Location captured — tap to refresh' : 'Tap to capture current location'}
                  </button>
                  {locationStatus && <p className={`mt-1 text-[11px] ${coordinates ? 'text-emerald-300' : 'text-amber-300'}`}>{locationStatus}</p>}
                </div>
              )}
            </>
          )}
