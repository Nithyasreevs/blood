...
import { ShieldAlert, MapPin, Clock, Zap, Bot, RefreshCw, XCircle, CheckCircle, Navigation } from 'lucide-react';
            ...
            <div key={req.request_id} className="glass-panel p-5 rounded-2xl border border-slate-700/80 hover:border-red-500/40 transition-all space-y-4">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5 flex items-center gap-3">
                      <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-red-400" /> {req.distance_km ?? '—'} km ({req.city || 'Unknown'})</span>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-slate-700/60">
                ...
                <div className="flex items-center gap-3">
                  {req.status === 'Donor Accepted' || req.status === 'Donor Arriving' ? (
                    <button onClick={() => onAcceptRequest?.(req.request_id)} className="btn-primary text-xs py-2 px-4 shadow-lg shadow-red-600/30">
                      <Navigation className="w-4 h-4" /> Continue Navigation
                    </button>
                  ) : <>
                    <button onClick={() => handleReject(req.request_id)} className="btn-secondary text-xs py-2 px-3 hover:border-red-500/50 hover:text-red-400">
                      <XCircle className="w-4 h-4" /> Decline
                    </button>
                    <button onClick={() => handleAccept(req.request_id)} className="btn-primary text-xs py-2 px-4 shadow-lg shadow-red-600/30">
