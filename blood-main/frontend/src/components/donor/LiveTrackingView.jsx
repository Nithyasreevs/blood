...
const LiveTrackingView = ({ requestId, user }) => {
  const [tracking, setTracking] = useState(null);
  const [loading, setLoading] = useState(true);
  const [completed, setCompleted] = useState(false);
  const [locationStatus, setLocationStatus] = useState('Updating your live location…');
  const lastSentLocation = useRef(null);

  const sendLocation = async (coords) => {
    if (!user?.user_id) return;
    const previous = lastSentLocation.current;
    if (previous && Math.abs(previous.latitude - coords.latitude) < 0.00005 && Math.abs(previous.longitude - coords.longitude) < 0.00005) return;
    lastSentLocation.current = coords;
    try {
      const res = await fetch('/api/donor/profile/update', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: user.user_id, latitude: coords.latitude, longitude: coords.longitude })
      });
      if (!res.ok) throw new Error('Location update failed');
      setLocationStatus(`Live GPS updated (accuracy about ${Math.round(coords.accuracy)} m)`);
    } catch {
      setLocationStatus('Could not update live GPS. Check your connection and location permission.');
    }
  };

  const fetchTracking = async () => {
    try {
      const donorId = user?.user_id ? `?donor_user_id=${encodeURIComponent(user.user_id)}` : '';
      const res = await fetch(`/api/request/tracking/${requestId}${donorId}`);
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="glass-panel p-6 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-emerald-300 mt-1">{locationStatus}</p>
        </div>
        <div className="bg-red-500/20 text-red-300 border border-red-500/40 text-xs px-3 py-1.5 rounded-xl font-mono font-bold">
          {tracking.distance_km == null ? 'Waiting for the donor location' : `ETA: ~${tracking.eta_minutes} Mins (${tracking.distance_km} km)`}
        </div>
      </div>
      ...
      <div className="glass-panel p-6 rounded-2xl bg-slate-950 border border-slate-700/80 space-y-6 relative overflow-hidden">
        ...
        <div className="relative z-10 p-4 rounded-xl bg-slate-900/90 border border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            {tracking.donor_lat != null && tracking.donor_lng != null && tracking.hospital_lat != null && tracking.hospital_lng != null && (
              <a
                href={`https://www.google.com/maps/dir/?api=1&origin=${tracking.donor_lat},${tracking.donor_lng}&destination=${tracking.hospital_lat},${tracking.hospital_lng}&travelmode=driving`}
                target="_blank"
                rel="noreferrer"
                className="btn-secondary text-xs py-2 px-4 flex items-center gap-2"
              >
                <Navigation className="w-4 h-4" /> Open Maps Route
              </a>
            )}
            <a href={`tel:${tracking.contact_number || '04428290200'}`} className="btn-primary text-xs py-2 px-4 shadow-lg shadow-red-600/30 flex items-center gap-2">
