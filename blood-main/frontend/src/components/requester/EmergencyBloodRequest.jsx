...
const EmergencyBloodRequest = ({ onRequestSubmitted }) => {
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          ...
          setLocationStatus(`Current location captured (accuracy about ${Math.round(pos.coords.accuracy)} m).`);
        },
        () => setLocationStatus('Location permission is required to find nearby donors.') ,
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    setErrorMsg('');
    if (!patientName || !phone || !units || !hospitalName || !selectedCity || !location) {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="form-group md:col-span-2">
                <label>Detected request location</label>
                <p className={`form-control flex items-center gap-2 ${location ? 'text-emerald-300' : 'text-amber-300'}`}>
                  <MapPin className="w-4 h-4" /> {location ? `${location.text} — GPS coordinates captured` : locationStatus}
                </p>
              </div>
