...
exports.register = async (req, res) => {
  try {
    ...
    if ((role === 'donor' || role === 'hospital') && !validCoordinates(latitude, longitude)) {
      return res.status(400).json({ success: false, message: 'A valid current location is required to calculate nearby matches.' });
    }

    const userLat = validCoordinates(latitude, longitude) ? Number(latitude) : undefined;
    const userLng = validCoordinates(latitude, longitude) ? Number(longitude) : undefined;
